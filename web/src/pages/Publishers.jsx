...
export default function Publishers() {
  ...
  useEffect(() => {
    const loadPublishers = async () => {
      try {
        const data = await fetchWithAuth(`${BASE_API_URL}/publishers`);
        const allPublishers = data.data || [];
        setPublishers(allPublishers);
        setFiltered(allPublishers);
  ...
  const handlePublisherClick = async (publisher) => {
    ...
    try {
      const data = await fetchWithAuth(`${BASE_API_URL}/publishers/${publisher.id}/articles`);
      setPublisherArticles(data.articles || []);
    } catch (error) {
      console.error("Failed to fetch articles for publisher:", error);
      setPublisherArticles([]);
    }
  };
        ...
        {!selectedPublisher ? (
          <>
            ...
            >
              ...
              >
                Search and browse all publishers found in your research database
              </p>
            </div>
            <div
              style={{
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((publisher) => (
                  ...
                  >
                    ...
                    >
                      {publisher.articleCount ?? 0} article
                      {publisher.articleCount === 1 ? "" : "s"}
                    </p>
                  </div>
                ))
              )}
          ...
          <div style={{ marginTop: "40px", animation: "fadeInDown 0.7s" }}>
            <div style={{
              ...
              <div>
                ...
                >
                  <span role="img" aria-label="publisher" style={{fontSize: "2.3rem"}}>📖</span>
                  Articles from <span style={{fontWeight:"800"}}>{selectedPublisher.name}</span>
                </h2>
                <p style={{
                  color: colors.mutedText,
                  fontSize: "1.1rem",
                  ...
                  Browse all articles published by this publisher
                </p>
              </div>
              <button
                onClick={() => {
              ...
              <table style={{
                ...
                <tbody>
                  {publisherArticles.length === 0 ? (
                    ...
                    publisherArticles.map((article, idx) => (
                      <tr
                        key={article.id}
                        style={{
